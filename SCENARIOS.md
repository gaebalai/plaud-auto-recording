...
2. 메인 화면 도달한 상태에서 `⌘⌥I`(개발자 도구) → **Application** 탭 → **Local Storage** → `https://web.plaud.ai`
3. `tokenstr` (또는 `token`, `access_token` 중 하나) 키의 값 클릭 → 길이가 긴 문자열 복사
4. `.env` 편집:
   ```bash
   PLAUD_AUTH_MODE=token
   PLAUD_TOKEN=eyJhbGc...복사한_토큰_전체
   ```
   (`PLAUD_EMAIL/PLAUD_PASSWORD`는 비워둬도 됨)
5. 시험 실행:
   ```bash
   npm run pipeline
   ```
