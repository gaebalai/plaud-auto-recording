...
본인 Chrome으로 평소처럼 로그인하고 토큰만 가져오는 방식:

1. **본인 Chrome**(자동화 아닌)에서 https://web.plaud.ai/ 접속, 평소처럼 구글 로그인
2. 메인 화면 도달한 상태에서 `⌘⌥I`(개발자 도구)
3. 토큰 가져오는 방법 두 가지:

   **방법 A — Network 탭에서 캡처 (⭐ 가장 확실)**
   - **Network** 탭 선택
   - 페이지 새로고침 (`⌘+R`)
   - 목록에서 `simple/web`, `filetag` 같은 요청 클릭
   - 우측 **Headers → Request Headers** 펼치기
   - `authorization: Bearer eyJ...` 라인의 `eyJ...` 부분 복사

   **방법 B — Local Storage에서 복사**
   - **Application** 탭 → Storage → Local Storage → `https://web.plaud.ai`
   - `tokenstr` 키 클릭 → 값 복사
   - ⚠ 일부 환경에서 Local Storage가 옛 토큰을 캐시하는 케이스가 있음. 401이 떨어지면 방법 A 사용

4. `.env` 편집:
   ```bash
   PLAUD_AUTH_MODE=token
   PLAUD_TOKEN=eyJhbGc...복사한_토큰_전체
   ```
   (`PLAUD_EMAIL/PLAUD_PASSWORD`는 비워둬도 됨. `Bearer ` prefix는 자동 제거됨)
...
다운로드가 401/403 에러로 실패 → "PLAUD 토큰 만료" 알림. 1~4단계를 다시 한 번 반복.
