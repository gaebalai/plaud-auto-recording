...
cron이 iCloud Drive(Obsidian Vault)에 쓰기 위해 필요합니다.

1. **시스템 설정** → **개인정보 보호 및 보안** → **전체 디스크 접근 권한**
2. 하단의 `+` 버튼 클릭
3. 파일 선택 창이 뜨면 **`⌘ + Shift + G`** (Command + Shift + G) 동시에 누르기
   - 작은 입력 창("폴더로 이동")이 나타남
   - 한글 입력기가 켜져 있으면 단축키가 안 먹힘 → 영문(ABC)으로 전환 후 시도
4. 입력란에 **`/usr/sbin/cron`** 입력 → Enter
5. `cron` 파일이 자동 선택된 상태로 열림 → **"열기"** 버튼 클릭
6. 목록에 추가된 cron의 토글을 **ON**

추가 권장: 같은 절차로 **`/bin/bash`** 도 추가해두면 더 안정적입니다.

`⌘⇧G`가 안 먹힐 때: Finder에서 `⌘⇧G` → `/usr/sbin` 입력 → cron 파일을 시스템 설정 창으로 끌어다 놓기.
...
## 시나리오 4-B. 구글 OAuth 자동 로그인이 차단된 경우 (token 모드)

### 증상
`npm run first-login`으로 브라우저가 떴는데 구글이 차단:
> "로그인할 수 없음 — 브라우저 또는 앱이 안전하지 않을 수 있습니다"

### 원인
구글이 Playwright의 자동화 표시(`navigator.webdriver` 등)를 탐지해서 차단. `session` 모드 사용 불가.

### 해결 — 토큰 직접 입력 모드

본인 Chrome에서 진짜 동작 토큰을 가져오는 세 가지 방법. 위로 갈수록 빠르고 자동화 정도가 높음.

#### 방법 1) ⭐ 자동 — 콘솔 인터셉트 + 클립보드 (npm run refresh-token)
1. **본인 Chrome**에서 https://web.plaud.ai 로그인 상태 확인
2. `⌘⌥I` → **Console** 탭
3. 터미널에서 `npm run refresh-token` 실행 → 표시된 한 줄을 콘솔에 붙여넣기 (Chrome 보안 경고 시 `Allow pasting` 입력)
4. Plaud Web 페이지를 한 번 클릭하거나 새로고침 → 콘솔에 "✅ 토큰 클립보드 복사" 알림
5. 터미널로 돌아와서 Enter → `pbpaste`로 클립보드 토큰 자동 사용 → API 검증 → `.env` 작성

#### 방법 2) 북마클릿 — 한 번 등록 후 클릭만으로 갱신
1. 프로젝트의 `tools/bookmarklet.html`을 브라우저로 열기 (`open tools/bookmarklet.html`)
2. 페이지의 "📋 PLAUD 토큰 추출" 버튼을 **북마크바에 드래그** (한 번만)
3. 이후 토큰 만료 시:
   - Plaud Web 메인 화면에서 북마크 클릭 → 알림 → 페이지 새로고침 → "✅ 클립보드 복사" 알림
   - 터미널에서 `npm run refresh-token` → Enter

#### 방법 3) 수동 — Network 탭에서 직접
1. `⌘⌥I` → **Network** 탭 → 페이지 새로고침
2. `simple/web` 요청 클릭 → Headers → Request Headers
3. `authorization: Bearer eyJ...` 의 `eyJ...` 부분만 복사
4. `.env` 편집:
   ```
   PLAUD_AUTH_MODE=token
   PLAUD_TOKEN=eyJhbGc...
   ```
5. `npm run pipeline`

> 💡 LocalStorage의 `tokenstr` 값은 옛 토큰이 캐시될 수 있어 비추천. Network 탭이나 위 방법 1·2가 진짜 동작 토큰을 보장함.
>
> `Bearer ` prefix와 앞뒤 공백은 모두 자동 정규화됩니다.

### 토큰 만료 시 (보통 수 주~수 개월 후)

다운로드가 401/403 에러로 실패 → "PLAUD 토큰 만료" 알림.

**가장 빠른 갱신 (1분)**:
1. Plaud Web 북마크바의 "📋 PLAUD 토큰 추출" 클릭 (또는 콘솔에 한 줄 붙여넣기)
2. 페이지 새로고침 → 클립보드에 새 토큰
3. 터미널에서 `npm run refresh-token` → Enter

### 헬스체크는?
```bash
npm run health
```
token 모드에서는 `.env`에 `PLAUD_TOKEN`이 있는지만 확인. 실제 만료는 다음 파이프라인 실행에서 검출됨. 매주 미리 알고 싶다면 token도 정기적으로 새로 복사하는 방법뿐.
...
1. 시스템 설정 → 개인정보 보호 및 보안 → 전체 디스크 접근 권한
2. 하단의 `+` 버튼 클릭
3. 파일 선택 창에서 **`⌘ + Shift + G`** (한글 입력기면 영문 ABC로 전환 후)
4. 입력란에 **`/usr/sbin/cron`** 입력 → Enter
5. cron 파일이 선택된 상태로 열림 → **"열기"** 클릭
6. 목록에 추가된 cron 항목의 토글을 **ON**
7. (보강) 같은 절차로 `/bin/bash`도 추가 권장

`⌘⇧G`가 안 먹힐 때: Finder에서 `⌘⇧G` → `/usr/sbin` → cron 파일을 시스템 설정 창에 드래그 앤 드롭.
